...
public class ReaderMessage  extends Message{
    ...
    public ReaderMessage(InputStream is,int fileLengthByteLength){
        ...
        try {
            super.setType((byte) is.read()); //读取第一个byte 作为文件的类型
            ...
            super.setContent(contentByteArray);
        } catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ReaderMessage(InputStream is){
        new ReaderMessage(is,4);
    }


    public Object objectDeserialize() {
        try {
            ObjectInputStream objectInputStream = new ObjectInputStream(new ByteArrayInputStream(getContent()));
